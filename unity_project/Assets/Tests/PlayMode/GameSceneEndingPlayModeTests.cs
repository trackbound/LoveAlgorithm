...
using LoveAlgo.Schedule; // ScheduleUI
using LoveAlgo.UI;       // EndingPanel
...
namespace LoveAlgo.Tests.PlayMode
{
    ...
    /// EndingPanel이 EnteredEndingEvent를 받아 엔딩 루트를 표시하는지 실 씬에서 검증.
    /// </summary>
    public class GameSceneEndingPlayModeTests
    {
        [UnityTest]
        public IEnumerator LastDay_Exhausted_Shows_EndingPanel()
        {
            yield return SceneManager.LoadSceneAsync("Game", LoadSceneMode.Single);
            yield return null; // 부팅 + UI OnEnable

            var ending = Object.FindAnyObjectByType<EndingPanel>(FindObjectsInactive.Include);
            Assert.IsNotNull(ending, "씬에 EndingPanel 존재");
