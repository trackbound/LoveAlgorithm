...
namespace LoveAlgo.Story.SaveSystem
{
    ...
    /// 캡처 시 그대로 유지되고, 그 외 UIManager/PopupSystem 하위 UI는 모두 숨겨진다.
    /// 캐릭터 CG, 배경 BG는 UIManager/PopupSystem 바깥에 있어 항상 유지됨.
    /// </summary>
    public static class SaveThumbnailSystem
    {
        ...
        public static async UniTask CapturePendingScreenshotAsync()
        {
            try
            {
                ...
                Debug.Log("[SaveThumbnailSystem] 팬딩 스크린샷 캡처 완료");
            }
            catch (Exception e)
            {
                Debug.LogWarning($"[SaveThumbnailSystem] 팬딩 스크린샷 실패: {e.Message}");
            }
        }
        ...
        public static void CapturePendingScreenshot()
        {
            try
            {
                ...
                UnityEngine.Object.Destroy(tex);
                UnityEngine.Object.Destroy(thumb);
                Debug.Log("[SaveThumbnailSystem] 팬딩 스크린샷 캡처 완료 (sync)");
            }
            catch (Exception e)
            {
                Debug.LogWarning($"[SaveThumbnailSystem] 팬딩 스크린샷 실패: {e.Message}");
        ...
        public static bool TryCommitPendingScreenshot(int slot)
        {
            ...
            {
                Debug.LogWarning($"[SaveThumbnailSystem] 썸네일 확정 실패: {e.Message}");
            }
            return false;
        }
        ...
        public static void CaptureScreenshot(int slot)
        {
            try
            {
                ...
                UnityEngine.Object.Destroy(tex);
                UnityEngine.Object.Destroy(thumb);
                Debug.Log($"[SaveThumbnailSystem] 슬롯 {slot} 스크린샷 저장");
            }
            catch (Exception e)
            {
                Debug.LogWarning($"[SaveThumbnailSystem] 스크린샷 저장 실패: {e.Message}");
            }
        }
        ...
        public static Sprite LoadScreenshot(int slot)
        {
            ...
            {
                Debug.LogWarning($"[SaveThumbnailSystem] 스크린샷 로드 실패: {e.Message}");
                return null;
            }
        }
        ...
        /// 유지(화이트리스트): 캐릭터 CG, 배경 BG(Stage 요소들 — UIManager/PopupSystem 하위가 아님),
        ///                     ScheduleUI, ShopUI (UIManager 하위).
        /// 숨김: 그 외 UIManager 하위 UI 전부, PopupSystem 하위 모든 팝업/레이어.
        /// </summary>
        static ThumbnailUISnapshot HideUIForThumbnailCapture()
        {
            ...
            var popups = PopupSystem.Instance;
            if (popups != null)
