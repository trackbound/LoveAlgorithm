namespace LoveAlgo.Events
{
    ...
    // 화면/흔들기 FX와 같은 구조: 순수 CameraFxParser가 FX Value를 인텐트로 분해 → 엔진(NarrativeController)이
    // 동결 수치(CameraFxTuningSO)로 duration을 해석 → 명령에 실어 발행 → 뷰(CameraFxView)가 _Stage 콘텐츠 래퍼의
    ...
    /// <see cref="CameraFxKind.Zoom"/>, <see cref="PanX"/>/<see cref="PanY"/>는 <see cref="CameraFxKind.Pan"/>에만 의미.
    /// </summary>
    public readonly struct CameraFxCommand
    {
        ...
        public CameraFxCommand(CameraFxKind kind, float zoomScale, float panX, float panY, float duration, CompletionHandle handle)
        {
            Kind = kind;
            ZoomScale = zoomScale;
            PanX = panX;
