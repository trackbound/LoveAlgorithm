...
namespace LoveAlgo.Story
{
    ...
    ///   <c>Flag:이름</c> / <c>!Flag:이름</c> · <c>Chose:태그</c> / <c>!Chose:태그</c>(선택 이력) · <c>Love:히로인{op}N</c> · <c>Stat:스탯{op}N</c> · <c>Meta:키{op}N</c>(메타 진행도/회차) · <c>스탯{op}N</c>(베어)
    ///   op ∈ { &gt;=  &lt;=  ==  &gt;  &lt; }
    ...
    public static class ConditionEvaluator
    {
        ...
        static bool EvaluateAtom(GameStateSO gs, string atom)
        {
            ...
            if (atom.StartsWith("Meta:",  StringComparison.Ordinal)) return Compare(atom.Substring(5), key => MetaProgressStore.GetInt(key)); // 메타 진행도(PlayerPrefs) — 회차 카운터 등(세이브 무관, 새 게임에도 생존)
