...
namespace LoveAlgo.UI
{
    ...
    /// 스프라이트(lid_top/lid_bottom)를 보간하고 완료 핸들을 푼다(ADR-007: UI는 표시만). DOTween 미사용.
    ///
    /// 직선 셔터가 아니라 진짜 눈꺼풀이 내려오/올라오도록:
    ///  1) 모양: 각 바는 화면 높이만큼 풀스크린 눈꺼풀이며, 맞닿는 가장자리가 완만히 중앙으로 처진 곡선(스프라이트).
    ///     상/하 곡선이 거울형이 아니라 동일(평행)이라 둘 사이가 균일폭 수평 슬릿으로 열려, 위·아래 눈꺼풀이 각자
    ///     제 방향으로 닫히는 느낌이 난다(아몬드/렌즈형 아님 — 그 조리개식 연출은 별도 FX용으로 분리 예정).
    ...
    public class EyeMaskView : MonoBehaviour
    {
        ...
        [Tooltip("상안검 주도 정도. 1=상하 대칭 닫힘, 클수록 윗눈꺼풀이 먼저·더 많이 내려오고 아랫눈꺼풀이 늦게 따라와 '눈꺼풀이 닫히는' 느낌. 두 바의 최종 위치는 불변이라 완전 암전은 유지.")]
