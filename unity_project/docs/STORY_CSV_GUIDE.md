...
- `Chose:태그` / `!Chose:태그` — 과거에 그 선택을 했는지 (선택지 옵션에 `mark:태그`로 표시 — 아래 "선택 마커")
...
**선택 마커(과거 선택 분기)**: 선택지 옵션 끝에 `|mark:태그`를 붙이면 그 보기를 고른 순간 이력에 기록된다. 이후 `If:Chose:태그`(또는 선택지 `if:Chose:태그`)가 읽어 분기한다. 플래그를 따로 설정할 필요 없이 "기억할 선택"을 한 토큰으로 명시 — 버튼 텍스트·점프 대상을 바꿔도 안 흔들린다. 세이브/로드를 거쳐도 유지된다.
- 옵션 형식: `버튼텍스트|점프대상|효과…|if:조건|mark:태그` (3번째+ 토큰은 순서 무관).
- 예: `,Option,,로아를 돕는다|help_roa|mark:helped_roa,>` → 이후 `,Flow,,If:Chose:helped_roa:roa_route,>`.
