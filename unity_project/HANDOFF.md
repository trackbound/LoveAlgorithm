...
> **현재(2026-06-05 末)**: 빌드 가능 기획자 스토리 에디터 + 작가 인게임 창 UX + WriterDelivery 빌드 전달 완료. **Prologue 재생 가능화 — ① 비주얼 백본 완성(48행: Setup·Wait·Scene·Place·Dialogue/Save·Video). 프롤로그가 일관되게 재생됨.** **즉시 다음 = 감독 프롤로그 Play 확인** → 남은 명령(graceful skip이라 재생 무차단)의 실제 필요 우선순위 결정: Value:Schedule(2, 의미=감독 결정)·LoadingScene(3, 새 뷰=Place식 배선)·LockScreen(1, ADR-013 Overlay 설계 동반). 그 후: ② 실 트리거 확장(Event2/3·Festival/MT/Confession 엔진 CSV + 카탈로그) ③ 시뮬 루프 심화. **아래는 구(舊) 다음-액션 목록 — 대부분 처리됨, 참고용.**
