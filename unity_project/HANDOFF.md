...
> **현재(2026-06-05 末)**: 빌드 가능 기획자 스토리 에디터 + 작가 인게임 창 UX + WriterDelivery 빌드 전달 완료. **Prologue 재생 가능화 — ① 비주얼 백본 완성(51행: Setup·Wait·Scene·Place·Dialogue/Save·Video·LoadingScene). 프롤로그가 일관되게 재생됨.** **즉시 다음 = 감독 입력 대기**(남은 2종 모두 추측 금지로 보류): ① **Value:Schedule**(2) 의미 결정 — 스케줄 페이즈 전환? 값 설정? 프롤로그선 다음 씬으로 바로 이어짐 ② **LockScreen**(1) = ADR-013 Overlay(완료-핸들 복귀) 설계. + **감독 프롤로그 Play 확인**(배너/로딩 레이아웃·폰트 튜닝). 그 후: ② 실 트리거 확장(Event2/3·Festival/MT/Confession 엔진 CSV + 카탈로그) ③ 시뮬 루프 심화. **아래는 구(舊) 다음-액션 목록 — 대부분 처리됨, 참고용.**
