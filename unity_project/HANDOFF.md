...
### 워크플로우 규율 (directive)
- **재설계 ≠ 전사(ADR-012 강화, 2026-06-02 감독 지적)**: 슬라이스 착수 시 **요구사항(REWRITE_FEATURE_INVENTORY §기능)에서 클래스 책임·데이터 흐름을 먼저 설계 — 구 코드 읽기 전에.** 구 코드는 동작 의미·동결 수치·CSV 문법(ADR-009) 확인용으로만, 구조/분해는 답습 금지. 커밋 "이식(port)" 표현 금지. 레드플래그: "이 클래스/필드가 요구사항 때문인가, 구 코드가 있어서인가?"
