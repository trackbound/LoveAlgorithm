...
> **현재(2026-06-05 末)**: 빌드 가능 기획자 스토리 에디터 + 작가 인게임 창 UX + WriterDelivery 빌드 전달 완료. **Prologue 재생 가능화 — ① 비주얼 백본 51행 + Value:Schedule no-op + LockScreen 파운데이션(FirstSetup: 세이브 필드·명령·파서) 완료.** **즉시 다음 = LockScreen 인터랙티브 슬라이스**: `LockScreenController`(Show+Submit→비번 저장·핸들 완료) + `LockScreenView`(평문 InputField) + Run 루프 `PlayLockScreen` 분기 + 씬배선. **⚠️ hang 주의 — 컨트롤러+뷰를 함께 배선하고 감독 Play로 비번입력을 수동검증해야**(작동 뷰 없이 Flow 배선하면 await가 LockScreen에서 hang). 그 후: 프롤로그 Play 확인(배너/로딩/잠금 레이아웃 튜닝)·다른 마일스톤(실 트리거 확장·시뮬 루프). 그 후: ② 실 트리거 확장(Event2/3·Festival/MT/Confession 엔진 CSV + 카탈로그) ③ 시뮬 루프 심화. **아래는 구(舊) 다음-액션 목록 — 대부분 처리됨, 참고용.**
